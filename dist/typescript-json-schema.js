    ...
    JsonSchemaGenerator.prototype.getDefinitionForRootType = function (propertyType, reffedType, definition, defaultNumberType, ignoreUndefined) {
        ...
        if (tupleType) {
            ...
            if (flags & ts.TypeFlags.String) {
                ...
                if (value !== undefined) {
                ...
                else if (propertyType.flags === typescript_1.TypeFlags.IndexedAccess) {
                    var indexedType = propertyType;
                    var objectType = indexedType.objectType;
                    var indexType = indexedType.indexType;
                    var resultingType = void 0;
                    if (objectType.flags & ts.TypeFlags.TypeParameter) {
                        var typeParameter = objectType;
                    ...
                    var indexedSymbolType = void 0;
                    if (!resultingType && objectType) {
                        if (indexType.flags & ts.TypeFlags.StringLiteral) {
                            var stringLiteralType = indexType;
                            var propertyName = stringLiteralType.value;
                            var indexedSymbol = objectType.getProperty(propertyName);
                            if (indexedSymbol) {
